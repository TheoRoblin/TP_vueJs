    ...
    <div class="bill" v-if="!loading && bill">
      <div class="row border-bottom pb-3 mb-3">
        ...
        <div v-if="!isNewBill" class="col text-end">
          <button @click="onDeleteBill(bill)" class="btn btn-outline-danger mx-2">
            <i class="fa-solid fa-trash me-2" />
            Supprimer la facture
          </button>
          <button class="btn btn-outline-primary">
            <RouterLink class="nav-link" aria-current="page" to="/bills"><i class="fa-solid fa-angle-left"/> Retour</RouterLink>
          </button>
        </div>
      </div>
    ...
    async onSave() {
      ...
      if (this.formInvalid) {
        ...
        if (this.isNewBill) {
          ...
          await this.createBill(this.bill)
        } else {
          console.log('update', this.bill)
          await this.updateBill(this.bill)
        }
